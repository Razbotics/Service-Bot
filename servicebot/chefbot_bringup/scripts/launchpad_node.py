...
#######################################################################################################################
		...
		self._ultrasonic_lowLimit = rospy.get_param("~ultrasonic_lowLimit", "24")
...
#######################################################################################################################
				...
				if(lineParts[0] == 'u'):
					self._ultrasonic_value = float(lineParts[1]) - float(self._ultrasonic_lowLimit)
...
#######################################################################################################################
				...
				if(lineParts[0] == 'i'):
					...
					self._qx = 0 #float(lineParts[1])
					self._qy = 0 #float(lineParts[2])
					self._qz = 0 #float(lineParts[3])
					self._qw = 0 #float(lineParts[4])


					self.ax = float(lineParts[1])
					self.ay = float(lineParts[2])
					self.az = float(lineParts[3])
					self.gx = float(lineParts[4])
					self.gy = float(lineParts[5])
					self.gz = float(lineParts[6])
